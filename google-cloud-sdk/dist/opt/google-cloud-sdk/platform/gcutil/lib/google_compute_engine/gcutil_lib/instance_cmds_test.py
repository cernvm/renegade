...
class InstanceCmdsTest(gcutil_unittest.GcutilTestCase):
  ...
  def testAddInstanceAndWaitUntilComplete(self):
    ...
    mock_lists.GetSampleImageListCall(command, self.mock, 10)
  ...
  def testAddInstanceWithExistingPersistentBootDisk(self):
    ...
    # Next, the user will be prompted to select an image.  Three calls happen.
    # One to the project, another to the CentOS and Debian projects.
    unused_imagecall_proj = mock_lists.GetSampleImageListCall(
        command, self.mock, 2)

    unused_imagecall_cent = mock_lists.GetSampleImageListCall(
        command, self.mock, 0)

    unused_imagecall_deb = mock_lists.GetSampleImageListCall(
        command, self.mock, 0)
  ...
  def testAddInstanceWithPersistentBootDisk(self):
    ...
    # Next, the user will be prompted to select an image.  Three calls happen.
    # One to the project, another to the CentOS and Debian projects, in
    # alphabetical order.
    unused_imagecall_cent = mock_lists.GetSampleImageListCall(
        command, self.mock, 0)

    unused_imagecall_proj = mock_lists.GetSampleImageListCall(
        command, self.mock, 2, name=['bad_image', 'my_image'])
  ...
  def testDeleteInstanceWithExistingPersistentBootDisk(self):
    ...
    self.assertEquals(1, len(ops_requests))
...
class OldInstanceCmdsTest(unittest.TestCase):
  ...
  def testAddInstanceWithDiskOptionsGeneratesCorrectRequest(self):
    ...
    submitted_disk_full_name = (
        'https://www.googleapis.com/compute/v1beta15/'
  ...
  def testAddInstanceWithSpecifiedInternalAddress(self):
    ...
    self.assertEqual(result['body']['description'], expected_description)
    self.assertEqual(result['body']['image'], expected_image)
    # Starting with v1beta15 server no longer supports setting an internal
    # network IP so gcutil no longer sends it to the server.
  ...
  def testBuildInstanceRequestWithMetadataAndDisk(self):
    ...
    disks = [{'source': ('https://www.googleapis.com/compute/v1beta15/projects/'
  ...
  def _DoTestAddAccessConfigGeneratesCorrectRequest(self, service_version):
    ...
    if command.api.version >= 'v1beta15':
