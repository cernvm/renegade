...
STANDARD_IMAGE_PROJECTS = ['debian-cloud', 'centos-cloud']
...
class Presenter(object):
  ...
  def PromptForImage(self, images_api):
    ...
    choices = sum((self._List(images_api, project)['items']
  ...
  def _List(self, api, project, per_zone=False):
    kwargs = {}
    if (per_zone and hasattr(self._flags, 'zone') and
        self._is_using_at_least_api_version('v1beta15')):
...
class GoogleComputeCommand(appcommands.Cmd):
  ...
  operation_print_spec = ResourcePrintSpec(
      summary=['name', 'status', 'operation-type'],
      field_mappings=(
          ('name', 'name'),
          ('zone', 'zone'),
          ('status', 'status'),
  ...
  operation_print_spec_v1beta15 = ResourcePrintSpec(
      summary=['name', 'status', 'operation-type'],
      field_mappings=(
          ('name', 'name'),
          ('region', 'region'),
  ...
  def GetOperationPrintSpec(self):
    if self.api.version >= 'v1beta15':
      return self.operation_print_spec_v1beta15
  ...
  def NormalizeMachineTypeResourceName(self, project, zone, collection,
    ...
    Returns:
      The full URL of the resource.
    """
    if self.api.version >= 'v1beta15' and zone:
...
class GoogleComputeListCommand(GoogleComputeCommand):
  ...
  def Handle(self):
    ...
    # Aggregated list.
    if (self.api.version >= 'v1beta15' and
        hasattr(self, 'ListAggregatedFunc') and
