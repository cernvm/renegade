...
class MoveInstancesBase(command_base.GoogleComputeCommand):
  ...
  def Handle(self, *args, **kwargs):
    ...
    if self.api.version > version.get('v1beta16'):
      raise gcutil_errors.UnsupportedCommand(
          'Moving instances is not supported in v1 yet. You may still use this '
          'subcommand by specifying --service_version=v1beta15 or '
  ...
  def _CreateSnapshots(self, snapshot_mappings, src_zone, dest_zone):
    ...
    for disk_name, snapshot_name in snapshot_mappings.iteritems():
      ...
      if self.api.version >= 'v1beta15':
        ...
        snapshot_resource['sourceDisk'] = self.NormalizePerZoneResourceName(
            self._project, src_zone, 'disks', disk_name)
        requests.append(self.api.snapshots.insert(
  ...
  def _CheckQuotas(self, instances_to_mv, disks_to_mv, src_zone, dest_zone,
    ...
    if self.api.version <= 'v1beta15':
      ...
      local_quotas = dest_zone_resource.get('quotas', [])
    else:
