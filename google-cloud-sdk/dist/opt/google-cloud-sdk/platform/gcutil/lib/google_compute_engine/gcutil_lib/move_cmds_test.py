...
class MoveInstancesTest(gcutil_unittest.GcutilTestCase):
  _SUPPORTED_API_VERSIONS = ('v1beta15', 'v1beta16')
  ...
  def testMoveInstanceWithReservedIpWithinRegion(self):
    ...
    elif self.api.version >= 'v1beta16':
      src_zone_response = {
          'kind': 'compute#zone',
          'name': expected_source_zone,
          'region': expected_region_url,
    ...
    # First, a call to self.api.projects.get happens.  The role of this is to
    # get quotas (we have none here) and to get externalIpAddresses in
    # v1beta14.
    if command.api.version >= 'v1beta15':
      ...
      project_call = self.mock.Respond(
          'compute.projects.get',
          {
              'kind': 'compute#project',
              'name': expected_project,
              'externalIpAddresses': [expected_reserved_ip],
              'quotas': [{
                  'limit': 8.0,
                  'metric': 'CPUS',
                  'usage': 2.0
    ...
    # Next, a call to self.api.addresses happens (for some API versions).
    if command.api.addresses:
...
class OldMoveInstancesBaseTestCase(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.api = old_mock_api.CreateMockApi()
...
class OldMoveInstancesTest(OldMoveInstancesBaseTestCase):
  ...
  def testFlagValidationWithNoSourceZone(self):
    self.flag_values.destination_zone = 'dest-zone'
    self.flag_values.service_version = 'v1beta15'
    self.assertRaises(app.UsageError, self.command.Handle, ['.*'])
