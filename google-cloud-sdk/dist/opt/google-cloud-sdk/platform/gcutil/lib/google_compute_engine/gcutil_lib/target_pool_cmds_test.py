...
class TargetPoolCmdsTests(gcutil_unittest.GcutilTestCase):
  ...
  def testLegacyRemoveTargetPoolInstanceGeneratesCorrectRequest(self):
    ...
    if self.api.version == version.get('v1beta15'):
      for instance in expected_instances:
        calls.append(self.mock.Respond('compute.targetPools.removeInstance',
                                       {}))
    elif self.api.version >= version.get('v1beta16'):
      calls.append(self.mock.Respond('compute.targetPools.removeInstance', {}))
    ...
    if self.api.version == version.get('v1beta15'):
      for call in calls:
        ...
        # Parse zone/instance from the request body
        actual_instances.append(instance)
    elif self.api.version >= version.get('v1beta16'):
        ...
        'items': {
            'zones/zone1': {
                'instances': [{
                    ...
                    'name': 'instance1',
                    'description': '',
                    'selfLink': 'https://www.googleapis.com/compute/v1beta15/'
  ...
  def testAddTargetPoolHealthCheckGeneratesCorrectRequest(self):
    ...
    # URLs will have an http/path prefix attached.
    if self.api.version == version.get('v1beta15'):
      self.assertTrue(body['healthCheck'].endswith('global/httpHealthChecks/' +
  ...
  def testGetTargetPoolPrintsInstances(self):
    ...
    url_prefix = ('https://www.googleapis.com/compute/v1beta15/projects/' +
