...
class UpdateManager(object):
  ...
  def __init__(self, sdk_root=None, url=None, platform_filter=None,
    ...
    if url:
      logging.warning('You are using an overridden snapshot URL: [%s]', url)
    else:
    ...
    self.__out_stream = out_stream if out_stream else sys.stdout
...
""".format('\n'.join(bad_commands)))
  ...
  def _DoFreshInstall(self, e):
    ...
    try:
      ...
      logging.error('An updated Cloud SDK failed to download')
      self._RaiseReinstallationFailedError()
